...
import { CalendarDays, Filter } from "lucide-react"; // Import Filter
import { useIsMobile } from "@/hooks/use-mobile"; // Import useIsMobile hook

// Mock data for merged timetable entries (from Lecturer + Course Rep-created timetables)
const mockMergedTimetableEntries = [
  {
    id: "mt1",
...
export default function ViewTimetable() {
  ...
  const filteredEntries = useMemo(() => {
    return mockMergedTimetableEntries.filter((entry) => {
      const matchesSearch =
  ...
  }, [searchTerm, selectedCourseCode, selectedDay, selectedStatus]);

  return (
    <div className="space-y-6">
      <Card className="dashboard-card">
        <CardHeader className="pb-3">
          <CardTitle className="text-lg">Your Timetable</CardTitle>
        </CardHeader>
        <CardContent className="pt-0 space-y-4">
          ...
          {filteredEntries.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-64 text-muted-foreground">
              ...
              <p className="text-sm">Your timetable will appear here.</p>
            </div>
          ) : (
                        ...
                        <span
                          className={`px-2 py-1 text-xs font-semibold rounded-full ${
                            entry.status === "Upcoming"
                              ...
                              : "bg-red-100 text-red-800" // Should not happen for students, but for completeness
                          }`}
                        >
                          {entry.status}
                        </span>
